...
----------------------------------------*/
function shoppableapplicationload(accountId, cdnUrl, appUrl, BaseURL, meta,myParam){
...
if  (myParam !== null){
      ...
      _initPlugin = function(el) {
        ...
        return jQueryObject(wrap).click(function(e) {
          ...
          if (wrap.data(umPrefix_+'image-tag-enabled')) {
            ...
            tagform = jQueryObject('<div class="'+umPrefix_+'tagform-wrapper">').append(jQueryObject(_this.options.tagForm).clone());
            //tagform.append('<a class="cancel-tag" href="#">Cancel</a>');

             // $('body').find('#tagform-wrapper').parent().css('z-index','6');
          ...
          /*  tagform.css({
            ...
            $('.'+umPrefix_+'image-tag-wrapper').css('z-index','5');
              wrap.css({
                  zIndex: '6'
              });
              $('#'+umPrefix_+'tab_comments').css("display", "block");
              $('.'+umPrefix_+'tab-comments').addClass(umPrefix_+'active');
              $('.'+umPrefix_+'tablinks').click(function(){
                  var id = $(this).attr("id");
                  $("#Type").val(id);
              });
      ...
      _attatch_tagform_events = function(formWrap) {
        ...
        formWrap.find('form').submit(function(e) {
          ...
          if (_check_required_fields(formWrap)) {
            ...
            formWrap.parent().find('.'+umPrefix_+'tagdiv.'+umPrefix_+'unsaved').text("Tag Done");
  ...
  forEachNode(document.images, function(elem) {   
    jQueryObject(elem).imageTag({tagForm: jQueryObject('#'+umPrefix_+'image2form'), labelProperty:'title'});
  });
